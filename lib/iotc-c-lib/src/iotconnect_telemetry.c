...
static cJSON* setup_telemetry_object(IotclMessageHandle message, const char* time)
{
    ...
    cJSON_AddStringToObject(telemetry_object, "dt", time);
    //cJSON *data_array = cJSON_AddArrayToObject(telemetry_object, "d");//Re-edit cJSON_AddObjectToObject
    ...
    if (!cJSON_AddItemToArray(message->telemetry_data_array, telemetry_object)) goto cleanup_da;//Re-edit

    // setup the actual telemetry object to be used in subsequent calls
    //message->current_telemetry_object = cJSON_CreateObject();//Re-edit
    if (!message->current_telemetry_object) goto cleanup_da;//Re-edit

    //if (!cJSON_AddItemToArray(data_array, message->current_telemetry_object)) goto cleanup_cto;//Re-edit

    return telemetry_object; // object inside the "d" array of the the root object

cleanup_cto:
    cJSON_free(message->current_telemetry_object);//Re-edit
...
IotclMessageHandle iotcl_telemetry_create() {
    ...
    if (!cJSON_AddStringToObject(msg->root_value, "dt", iotcl_iso_timestamp_now()))
    {
        goto cleanup;
    }
    /*
...
bool iotcl_telemetry_add_with_iso_time(IotclMessageHandle message, const char* time) {
    ...
    //if (!cJSON_AddStringToObject(telemetry_object, "dt", time)) return false;//Re-edit
    // if (!cJSON_HasObjectItem(message->root_value, "t")) {//Re-edit
    //     if (!cJSON_AddStringToObject(message->root_value, "t", time)) return false;//Re-edit
   //  }//Re-edit
